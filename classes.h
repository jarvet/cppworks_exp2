...
public:
    ...
    int CurveLen();//计算并输出折线长度

    //重载运算符
    Curve3D operator+(Point3D t_point);//将一个Point3D点增加到折线点序列中
    Curve3D operator-(Point3D t_point);//将一个Poin3D点从折线序列中删除
    Curve3D operator=(Point3D t_point);//浅拷贝
